...
// ---------- FOR THE RIGHT HAND ---------------------------------------------------------------
			...
			listener.lookupTransform("/head_1", "/right_hand_1", ros::Time(0), transform1);
...
// --------------------- FOR THE LEFT HAND ------------------------------------------------------			
			...
			listener.lookupTransform("/torso_1", "/right_elbow_1", ros::Time(0), transform2);
...
// --------------------- FOR THE HEAD ------------------------------------------------------			
			...
			listener.lookupTransform("/left_hip_1", "/left_hand_1", ros::Time(0), transform2);
...
//putting the values in vector to save
    ...
    x_file.open ("wave_June16.csv", ios::app); //the flag ios::app makes sure that the control starts at the end of already existing file.
		x_file << 8;
		x_file << ",";
